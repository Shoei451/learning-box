...
// Supabase: 取得
// ─────────────────────────────────────────────
// Supabase の1リクエストで取得する上限（PostgRESTのデフォルトは1000）
// 学習用途では全件メモリに持つ設計。データが非常に多い場合は
// SUPABASE_TABLE に RLS + category フィルタを設けてサーバー側で絞るのが理想。
const SB_FETCH_LIMIT = 2000;

async function loadFromSupabase(showStatus) {
  setStatus('読み込み中…', '');
  try {
    // 必要な列だけ指定 + limit で上限を明示
    const url = `${SUPABASE_URL}/rest/v1/${SUPABASE_TABLE}`
      + `?select=id,category,question,answer,explanation,image_url`
      + `&order=id.asc`
      + `&limit=${SB_FETCH_LIMIT}`;
    ...
    const limitNote = data.length >= SB_FETCH_LIMIT ? `（上限 ${SB_FETCH_LIMIT} 件）` : '';
    setStatus(`✓ ${allCards.length} 件取得${limitNote}`, 'ok');
    buildFilters();
    updateStartMeta();
    renderQList();
  } catch (e) {
...
// ── 問題一覧（最大 QLIST_DISPLAY 件を表示。検索で絞り込み可）──
const QLIST_DISPLAY = 200;
let qListQuery = '';
...
function renderQList() {
  ...
  const q = qListQuery.trim().toLowerCase();
  const filtered = q
    ? allCards.filter(c =>
  ...
  const shown = filtered.slice(0, QLIST_DISPLAY);
  const over  = filtered.length - shown.length;

  if (cnt) cnt.textContent = `(${filtered.length}${q ? ' 件該当' : ''})`;
  ...
  `).join('')
  + (over > 0 ? `<p class="qlist-over">… 他 ${over} 件（検索で絞り込み）</p>` : '');
}
