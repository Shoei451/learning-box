...
function renderCard() {
  ...
  [
    document.getElementById("frontBadge"),
    document.getElementById("backBadge"),
  ].forEach((b, i) => {
    b.textContent = label;
    if (i === 0) {
...
function markCard(level) {
  ...
  renderList();
  const btn =
    document.querySelectorAll(".mastery-btn")[
      { forgot: 0, unsure: 1, knew: 2 }[level]
    ];
...
// ══════════════════════════════════════════════
function resetForm() {
  ["f_question", "f_answer", "f_explanation", "f_imageUrl"].forEach(
...
function renderQList() {
  ...
  activeDefs()
    .filter((f) => f.id !== "all")
    .forEach((f) => {
      groups[f.id] = { label: f.label, items: [] };
    });
  allCards.forEach((c) => {
    if (!groups[c.category])
      groups[c.category] = { label: c.category, items: [] };
